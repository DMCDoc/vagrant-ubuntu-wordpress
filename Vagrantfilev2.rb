...
# Télécharger les nouvelles clés WordPress
sudo -u www-data curl -s https://api.wordpress.org/secret-key/1.1/salt/ > /tmp/wp-keys

# Vérifier le contenu du fichier temporaire
cat /tmp/wp-keys
cat /srv/www/wordpress/wp-config.php

# Remplacement des clés dans wp-config.php
while IFS= read -r line; do
    key=$(echo "$line" | awk -F"'" '{print $2}')  # Extrait le type de clé (AUTH_KEY, etc.)

    # Échapper tous les caractères spéciaux pour sed
    escaped_line=$(echo "$line" | sed -E 's/[\&/|`$.*+?{}()\^[]/\\&/g')

    # Remplacement sécurisé dans wp-config.php
    sudo sed -Ei "s|define\('$key', '.*'\);|$escaped_line|g" /srv/www/wordpress/wp-config.php
done < /tmp/wp-keys
